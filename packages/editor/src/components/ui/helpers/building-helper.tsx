  ...
  return (
    <div className="pointer-events-none fixed top-1/2 right-4 z-40 flex -translate-y-1/2 flex-col gap-2 rounded-lg border border-border bg-background/95 px-4 py-3 shadow-lg backdrop-blur-md">
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className="text-muted-foreground">{t('editor.placeBuilding')}</span>
      </div>
      {showRotate && (
        <>
          <div className="flex items-center gap-2 text-sm">
            <ShortcutToken value="R" />
            <span className="text-muted-foreground">{t('editor.rotateCounterclockwise')}</span>
          </div>
          <div className="flex items-center gap-2 text-sm">
            <ShortcutToken value="T" />
            <span className="text-muted-foreground">{t('editor.rotateClockwise')}</span>
          </div>
        </>
      )}
      <div className="flex items-center gap-2 text-sm">
